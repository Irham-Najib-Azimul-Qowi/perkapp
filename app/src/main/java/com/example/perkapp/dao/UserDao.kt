...
import com.example.perkapp.model.UserEntity // Mengimpor entitas UserEntity
...
interface UserDao { // Deklarasi interface UserDao
    ...
    /**
     * FUNGSI: getUserByEmailOrName
     * TUJUAN: Mengambil data pengguna berdasarkan email atau nama secara *offline*.
     */
    @Query("SELECT * FROM users WHERE email = :email OR name = :email LIMIT 1")
    suspend fun getUserByEmailOrName(email: String): UserEntity?
