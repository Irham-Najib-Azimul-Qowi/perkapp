...
import com.example.perkapp.util.NetworkUtils
...
class AlatRepository(
    ...
    suspend fun createAlat(name: String, category: String, totalQty: Int, condition: String, imagePath: String?) {
        ...
        if (!imagePath.isNullOrBlank()) {
            try {
                val db = com.example.perkapp.database.AppDatabase.getDatabase(context)
                val mediaApi = com.example.perkapp.network.RetrofitClient.instance.create(com.example.perkapp.network.MediaApiService::class.java)
                val imageDao = db.imageDao()
                val mediaRepository = com.example.perkapp.repository.MediaRepository(mediaApi, imageDao, context)
    ...
    suspend fun syncPendingData(): Boolean {
        ...
        return syncMutex.withLock {
            ...
            try {
                val db = com.example.perkapp.database.AppDatabase.getDatabase(context)
                val mediaApi = com.example.perkapp.network.RetrofitClient.instance.create(com.example.perkapp.network.MediaApiService::class.java)
                val mediaRepository = com.example.perkapp.repository.MediaRepository(mediaApi, db.imageDao(), context)
